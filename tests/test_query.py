...
def test_build_sortable_filter_with_default_field(default_query):
    ...
    assert default_query._sortable_filter == [("_id", ASCENDING)]


def test_build_sortable_filter_with_custom_field(custom_query):
    custom_query._build_sortable_filter(ASCENDING)
    assert custom_query._sortable_filter == [("custom_field", ASCENDING), ("_id", ASCENDING)]
...
def test_build_mongo_filter_with_default_field(default_query):
    ...
    assert default_query._mongo_filter == {"_id": {"mongo_key": ObjectId("5ee7cb5dfce77c2d2b9d84f4")}}
...
def test_build_mongo_filter_with_custom_field(custom_query):
    ...
    assert custom_query._mongo_filter == {
        "$or": [
            {"custom_field": {"mongo_key": "custom_field"}},
            {"custom_field": "custom_field", "_id": {"mongo_key": ObjectId("5ee7cb5dfce77c2d2b9d84f4")}},
        ]
    }
...
@pytest.mark.parametrize(
    "query, prev_page, next_page, ordering_case, field, result",
    [(None, None, None, ASCENDING, DEFAULT_ORDERING_FIELD, (ASCENDING,))],
)
...
@pytest.mark.parametrize(
    "query, prev_page, next_page, ordering_case, field, result",
    [
        (None, "prev_page", None, ASCENDING, DEFAULT_ORDERING_FIELD, ("prev_page", False)),
        (None, None, "next_page", ASCENDING, DEFAULT_ORDERING_FIELD, ("next_page", True)),
    ],
)
def test_build_query_with_page_pointers(query, prev_page, next_page, ordering_case, field, result):
